...
universe and take new entries on qualifying breakouts.
...
def manage_exits(config: Config, broker: BrokerInterface, state: StrategyState, universe: dict) -> None:
    for pos in state.all_open():
        ...
        if exit_price is not None:
            ...
            broker.place_order(pos.symbol, order_side, pos.quantity, product=config.order_product)
...
def find_new_entries(config: Config, broker: BrokerInterface, state: StrategyState, universe: dict) -> list[str]:
    ...
    for result in shortlisted:
        ...
        broker.place_order(result.symbol, order_side, quantity, product=config.order_product)
        state.open_position(
...
def run_daily_cycle(config: Config | None = None) -> None:
    ...
    logger.info("=== swing agent run start (%s), mode=%s ===",
                datetime.now(timezone.utc).isoformat(), "LIVE" if config.is_live else "PAPER")
    ...
    manage_exits(config, broker, state, universe)
    opened = find_new_entries(config, broker, state, universe)
